<?php
    ...
    $stmt = $conn->prepare("SELECT security_notifications, comp_email, comp_name FROM store_settings LIMIT 1");
    $stmt->execute();
    ...
    if (empty($_POST['npassword'])) {
        ...
        if (password_verify($_POST['opassword'], $my_password)) {
            ...
            if (strlen($_POST['npassword']) < 8) {
                ...
                $error = true;
            } elseif ($_POST['opassword'] === $_POST['npassword']) {
                $data = "New password must be different from current password";
                $error = true;
            } else {
                $update_pass=$conn->prepare("UPDATE users SET `password`=:my_password, updated_at=NOW() WHERE id=:id");
                ...
                if($update_pass->execute()){
                    ...
                    if($site_settings->security_notifications == "1"){
                        ...
                        $subject = "🔐 Password Successfully Updated - {$brandName}";
                        ...
                        $message = <<<HTML
                            ...
                            <head>
                                ...
                                <title>Password Updated - {$brandName}</title>
                            </head>
                                ...
                                <div style="max-width: 600px; margin: 0 auto; background: white; border-radius: 20px; overflow: hidden; box-shadow: 0 12px 40px hsla(20,30%,20%,0.15);">
                                    ...
                                    <div style="background: linear-gradient(135deg, hsl(20,85%,50%) 0%, hsl(15,70%,55%) 100%); padding: 50px 30px; text-align: center; position: relative; overflow: hidden;">
                                        ...
                                        <p style="font-size: 16px; color: rgba(255, 255, 255, 0.95); margin-top: 10px; position: relative; z-index: 2;">
                                            Your {$brandName} account is now more secure
                                        </p>
                                    </div>
                                    ...
                                    <div style="padding: 50px 40px;">
                                        ...
                                        <div style="margin-bottom: 30px;">
                                            ...
                                            <p style="color: hsl(20,20%,45%); font-size: 16px; margin: 0;">
                                                Your {$brandName} account password has been successfully updated. Here's what you need to know:
                                            </p>
                                        </div>
                                        ...
                                        <div style="margin-top: 40px; padding-top: 25px; border-top: 1px solid hsl(30,20%,88%); text-align: center;">
                                            <p style="color: hsl(20,20%,45%); margin: 0 0 15px 0; font-size: 15px;">
                                                Thank you for helping us keep your {$brandName} account secure.
                                            </p>
                        ...
                        if ($my_details->sms_alerts == '1' && !empty($my_details->mobile_number)) {
                            $smsMessage = "🔐 Your {$brandName} password has been updated successfully. If this wasn't you, please secure your account immediately.";
