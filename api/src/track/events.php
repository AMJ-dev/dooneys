<?php
    ...
    try {
        ...
        $stmt = $conn->prepare("
            INSERT INTO events (visitor_id, session_id, event, entity_id, value, metadata)
            VALUES (:visitor_id, :session_id, :event, :entity_id, :value, :metadata)
        ");
        $stmt->bindValue(":visitor_id", $visitor_id, PDO::PARAM_INT);
        $stmt->bindValue(":session_id", $session_id, PDO::PARAM_INT);
        $stmt->bindValue(":event", $event, PDO::PARAM_STR);
        $stmt->bindValue(":entity_id", $entity_id, $entity_id ? PDO::PARAM_INT : PDO::PARAM_NULL);
        $stmt->bindValue(":value", $value, $value !== null ? PDO::PARAM_STR : PDO::PARAM_NULL);
        $stmt->bindValue(":metadata", $metadata ? json_encode($metadata) : null, $metadata ? PDO::PARAM_STR : PDO::PARAM_NULL);
        $stmt->execute();
